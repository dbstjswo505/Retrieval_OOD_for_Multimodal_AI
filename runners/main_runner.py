...
class MainRunner:
    def __init__(self, args):
        ...
        with open(self.args['dataset']['test_diff'], "r") as f:
            self.vid_difficulty = json.load(f)
        self._build_model()
        if 'train' in args:
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                for bid, batch in enumerate(self.test_loader, 1):
                    ...
                    video_len  = batch['raw'][0][1]
                    video_comp = batch['raw'][0][-1]
                    if video_id not in analysis.keys():
                        analysis[video_id] = [video_len, video_comp]
                    ...
                    res5 = res
                    analysis[video_id].append(res1['mIoU'])
                    print(bid)
                    for key, v in res.items():
                        metrics_logger['R@%d,'%(k)+key].update(v, bsz)
#                    if type(self.vid_difficulty[video_id]) == type(list()):
#                        self.vid_difficulty[video_id] = self.vid_difficulty[video_id] + [res1]
#                    else:
#                        self.vid_difficulty[video_id] = [self.vid_difficulty[video_id], res1]
#                    print(video_id)
#            with open("video_difficulty_analysis.json", "w") as json_file:
#                json.dump(self.vid_difficulty, json_file)
            with open("prediction_with_comp.json","w") as json_file:
